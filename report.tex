...
\author{Tên: Nguyễn Công Nguyên\\MSSV: 24120106 \\Môn: Toán ứng dụng và thống kê}
\date{\vspace{-5mm}}
...
Gauss-Seidel là phương pháp lặp dùng ngay giá trị mới vừa tính được trong cùng một vòng lặp, vì vậy tốc độ hội tụ thường tốt hơn Jacobi trên cùng một hệ. Với hệ $A\mathbf{x}=\mathbf{b}$, công thức cập nhật cho phần tử thứ $i$ là:

\[
x_i^{(k+1)} = \frac{1}{a_{ii}}\left(b_i - \sum_{j < i} a_{ij}x_j^{(k+1)} - \sum_{j > i} a_{ij}x_j^{(k)}\right)
\]

Trong đó, các ẩn đã cập nhật ở vòng lặp hiện tại $x_0^{(k+1)},\dots,x_{i-1}^{(k+1)}$ được dùng ngay lập tức, còn các ẩn chưa cập nhật $x_{i+1}^{(k)},\dots,x_{n-1}^{(k)}$ vẫn lấy từ vòng lặp trước. Đây là điểm khác biệt giúp Gauss-Seidel tận dụng tốt thông tin mới hơn trong quá trình lặp.

Về mặt cài đặt, hàm \texttt{gauss\_seidel(A, b, x0, tol, max\_iter, eps)} xử lý theo các bước:
\begin{enumerate}
    \item Kiểm tra tính hợp lệ của hệ bằng \texttt{validate\_system(A, b)} để đảm bảo ma trận vuông và vector vế phải có kích thước phù hợp.
    \item Khởi tạo nghiệm ban đầu bằng vector không nếu không truyền \texttt{x0}. Nếu người dùng cung cấp \texttt{x0}, hàm sẽ sao chép vector này để tránh sửa trực tiếp dữ liệu gốc.
    \item Kiểm tra xem ma trận có chéo trội hàng nghiêm ngặt hay không thông qua \texttt{is\_strictly\_row\_diagonally\_dominant(A)}. Kết quả này chỉ được lưu vào thông tin trả về để tham khảo, không dùng để chặn thuật toán.
    \item Lặp tối đa \texttt{max\_iter} lần, cập nhật từng biến theo đúng công thức Gauss-Seidel và dừng khi sai khác giữa hai vòng lặp liên tiếp đủ nhỏ.
\end{enumerate}

Điều kiện dừng trong code không dựa trực tiếp vào residual mà dựa trên chuẩn vô cùng của sai khác giữa hai nghiệm liên tiếp:

\[
\|\mathbf{x}^{(k+1)} - \mathbf{x}^{(k)}\|_{\infty} = \max_i \left|x_i^{(k+1)} - x_i^{(k)}\right| < \texttt{tol}
\]

Nếu đạt điều kiện trên, hàm trả về \texttt{converged = True} cùng số vòng lặp đã dùng. Nếu chạy hết \texttt{max\_iter} mà vẫn chưa đạt ngưỡng, hàm trả về \texttt{converged = False} để báo rằng quá trình lặp chưa hội tụ trong giới hạn cho phép.

Để tránh chia cho phần tử đường chéo gần bằng 0, mỗi bước cập nhật đều kiểm tra \texttt{|a\_ii| < eps}. Nếu gặp trường hợp này, hàm sẽ ném ngoại lệ vì hệ không phù hợp cho Gauss-Seidel theo cách cài đặt hiện tại.

Ngoài nghiệm, hàm còn trả về một \texttt{dict} chứa ba thông tin: số vòng lặp \texttt{iterations}, trạng thái hội tụ \texttt{converged}, và cờ \texttt{diag\_dominant}. Những thông tin này rất hữu ích khi benchmark vì cho biết không chỉ nghiệm cuối cùng mà còn cho thấy phương pháp cần bao nhiêu bước để ổn định trên từng bộ dữ liệu.

Về tính hội tụ, Gauss-Seidel thường làm việc tốt hơn khi ma trận đối xứng xác định dương hoặc chéo trội hàng nghiêm ngặt. Ngược lại, nếu ma trận không thỏa các điều kiện này thì phương pháp vẫn có thể chạy, nhưng tốc độ hội tụ hoặc thậm chí khả năng hội tụ không được đảm bảo.
